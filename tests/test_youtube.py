...
def test_panel_appears_on_yt_home(yt_home_page: Page):
    """Extension injects the control panel on YouTube home."""
    expect(yt_home_page.locator("[data-doom-panel]")).to_have_count(1)


def test_yt_home_doom_post_after_new_videos(yt_home_page: Page):
    """
    Adding ≥12 ytd-rich-item-renderer elements triggers the 2-s poll and
    eventually fires a doom post.
    """
    yt_home_page.evaluate(
        "document.querySelectorAll('[data-doom-scroll-post]').forEach(el => el.remove())"
    )

    for i in range(12):
        yt_home_page.evaluate("""(i) => {
        ...
        yt_home_page.wait_for_timeout(100)

    # Poll runs every 2 s; give up to 10 s for at least one doom post
    yt_home_page.wait_for_selector("[data-doom-scroll-post]", timeout=10_000)
    count = yt_home_page.locator("[data-doom-scroll-post]").count()
    assert count >= 1


def test_yt_home_panel_shows_youtube_site(yt_home_page: Page):
    """Control panel chip shows 'youtube'."""
    chip_text = yt_home_page.evaluate("""() => {
...
def test_panel_appears_on_yt_watch(yt_watch_page: Page):
    """Extension injects the control panel on YouTube watch page."""
    expect(yt_watch_page.locator("[data-doom-panel]")).to_have_count(1)


def test_yt_watch_doom_post_in_sidebar(yt_watch_page: Page):
    """Adding suggestion cards in the sidebar triggers the post counter."""
    yt_watch_page.evaluate(
        "document.querySelectorAll('[data-doom-scroll-post]').forEach(el => el.remove())"
    )

    for i in range(12):
        yt_watch_page.evaluate("""(i) => {
        ...
        yt_watch_page.wait_for_timeout(100)

    yt_watch_page.wait_for_selector("[data-doom-scroll-post]", timeout=10_000)
    count = yt_watch_page.locator("[data-doom-scroll-post]").count()
    assert count >= 1
...
def test_yt_spa_navigation_resets_post_counter(yt_home_page: Page):
    """
    ...
    yt_home_page.wait_for_timeout(3_000)
    ...
    yt_home_page.wait_for_timeout(500)
